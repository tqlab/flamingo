...
fn set_device_mtu(ifname: &str, mtu: usize) -> io::Result<()> {
    ...
    let mut ifreq = IfReq::new(ifname, mtu);
...
fn get_device_addr(ifname: &str) -> io::Result<Ipv4Addr> {
    let sock = UdpSocket::bind("0.0.0.0:0")?;
    let mut ifreq = IfReq::new(ifname, MaybeUninit::<IfReqDataAddr>::uninit());
    let res = unsafe { libc::ioctl(sock.as_raw_fd(), libc::SIOCGIFADDR.try_into().unwrap(), &mut ifreq) };
    match res {
        0 => {
            let data = unsafe { ifreq.data.assume_init() };
            Ok(data.addr)
        }
        _ => Err(IoError::last_os_error()),
    }
...
fn set_device_enabled(ifname: &str, up: bool) -> io::Result<()> {
    ...
    let mut flag: libc::c_int = unsafe { ifreq.data.assume_init() };
    if up {
        flag |= libc::IFF_UP | libc::IFF_RUNNING
    } else {
        flag &= !libc::IFF_UP
    }
    let mut ifreq = IfReq::new(ifname, flag);
