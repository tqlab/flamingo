...
pub fn configure(name: Option<String>) -> Result<(), dialoguer::Error> {
    ...
    let name = if let Some(name) = name {
        ...
        let mut names = vec![];
        for file in fs::read_dir("/etc/flamingo")? {
    ...
    let file = Path::new("/etc/flamingo").join(format!("{}.net", name));
    if file.exists() {
    ...
    if Confirm::with_theme(&theme).with_prompt("Save config?").default(true).interact()? {
        ...
        println!("  start the VPN:   sudo service flamingo@{0} start", name);
        println!("  stop the VPN:    sudo service flamingo@{0} stop", name);
        println!("  get the status:  sudo service flamingo@{0} status", name);
        println!("  add VPN to autostart:       sudo systemctl enable flamingo@{0}", name);
        println!("  remove VPN from autostart:  sudo systemctl disable flamingo@{0}", name);
    }
